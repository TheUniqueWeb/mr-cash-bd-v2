...
drizzleDb.select().from(systemSettings).where(eq(systemSettings.id, 'global')).limit(1)
  ...
  async function checkIpBangladesh(ip: string): Promise<{ isBD: boolean; isProxy: boolean; country: string; countryCode: string }> {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const countryCode = data.country_code || 'BD'; // "BD"
  ...
  // CPALead High-Performance In-Memory Cache
  interface OffersCacheItem {
    offers: any[];
    timestamp: number;
  }
  const offersCacheByCountry: Record<string, OffersCacheItem> = {};
  const OFFERS_CACHE_TTL_MS = 2 * 60 * 1000; // Cache CPALead results for 2 minutes to ensure high performance

  // 4. Offerwall fetcher (CPALead integration with high performance caching)
  app.get('/api/v1/offers', async (req, res) => {
    try {
      ...
      const now = Date.now();
      const cached = offersCacheByCountry[cacheKey];

      let formattedOffers: any[] = [];

      // Serve from high-performance cache if fresh (under 2 minutes old)
      if (cached && (now - cached.timestamp < OFFERS_CACHE_TTL_MS)) {
        ...
        try {
          ...
          if (cached) {
            console.log('Serving stale cache for country:', cacheKey);
            formattedOffers = cached.offers;
          }
        }
      }
  ...
  const handleRedirect = (req: any, res: any) => {
    try {
